...
function createIdentityRoutes(didService, db) {
    const router = (0, express_1.Router)();
    // GET /api/identities
    router.get('/', (_req, res) => {
        try {
            const identities = db.query('SELECT * FROM identities ORDER BY created_at DESC');
            const users = db.query('SELECT id, email, display_name, user_type, did, status FROM users');
            const organizations = db.query('SELECT * FROM organizations');
            res.json({
                success: true,
