...
export class AnomalyRiskEngine {
   ...
   */
  public assessTrustObjectRisk(params: {
    ...
    const duplicateSubjectRow = this.db.getOne<{ count: number }>(
      `SELECT COUNT(DISTINCT subject_id) as count 
       FROM trust_objects 
       WHERE content_hash = ? AND trust_object_id != ?`,
      [params.trustObject.contentHash, params.trustObject.trustObjectId]
    );
    ...
    const recentVerifications = this.db.getOne<{ count: number }>(
    ...
    if (params.trustObject.objectType === 'DEVICE') {
      const securityEventRow = this.db.getOne<{ count: number }>(
        `SELECT COUNT(*) as count 
         FROM security_events 
         WHERE device_id = ? AND remediated = 0`,
        [params.trustObject.subjectId]
      );
